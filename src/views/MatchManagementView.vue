...
<template>
  <div class="p-6 max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold mb-4 flex items-center space-x-2">
      <FontAwesomeIcon :icon="faSoccerBall" class="text-green-600" />
      <span>Gestion des Matchs</span>
    </h1>

    <!-- Formulaire pour créer un match -->
    <form @submit.prevent="createMatch" class="space-y-6 mb-8 bg-gray-50 p-4 rounded shadow-lg">
      <div>
        <label for="activity" class="block text-sm font-medium text-gray-700">Activité</label>
        <select
          v-model="matchForm.activityId"
          class="border p-2 w-full text-sm rounded-lg shadow-sm"
        >
      ...
      <div>
        ...
        <select v-model="matchForm.team2Id" class="border p-2 w-full text-sm rounded-lg shadow-sm">
      ...
      <button
        type="submit"
        class="bg-blue-500 text-white p-2 rounded text-sm flex items-center space-x-2 hover:bg-blue-600"
      >
    ...
    <div class="bg-white p-4 rounded shadow-lg">
      <h2 class="text-xl font-bold mb-4 flex items-center space-x-2">
        <FontAwesomeIcon :icon="faList" class="text-gray-700" />
        <span>Tous les matchs</span>
      </h2>
      <table class="table-auto w-full border-collapse text-sm">
        <thead>
          <tr>
            <th class="border px-4 py-2 text-left">Équipe 1</th>
            <th class="border px-4 py-2 text-left">Équipe 2</th>
            <th class="border px-4 py-2 text-left">Activité</th>
